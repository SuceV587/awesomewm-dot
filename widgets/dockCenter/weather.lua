...
M_dock_center_weather = {
	...
	init = function()
		...
		local widget = M_dock_center_weather:make_widget_wrap()
	...
	fetchApi = function(args)
		local callback = function()
			...
			local current_call = args.current_call
				or "curl  -L -X GET --compressed 'https://devapi.qweather.com/v7/weather/7d?location=%s&key=%s' "
	...
	loopWeather = function(args)
		utils.run.newtimer(
			"weather-fetch",
			M_dock_center_weather.timeout(args),
			M_dock_center_weather.fetchApi(args),
			false,
			true
		)
	end,
	make_widget_wrap = function()
		...
		today_location_widget = wibox.widget({
			text = constants.weather_city_name or "北京",
	...
	loopFutureWeather = function(future_widget, data)
		...
		for i, v in ipairs(data) do
			...
			--天气图标
			local icons = constants.assets .. "/icons/" .. v.iconDay .. ".svg"
			--设置天气图标大小为20x20
