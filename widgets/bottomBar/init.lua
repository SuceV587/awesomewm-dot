...
M_bottom_bar = {
	init = function(screen)
		...
		if screen.index == 1 then
			...
			-- 	M_bottom_bar.listen_event(bottom_nav_line, bottom_dock, screen)
		end
	end,
	--创建底部的dock栏
					...
					wibox.widget({
						awful.widget.tasklist({
							...
							widget_template = {
								...
								id = "background_role",
								forced_width = utils.ui.dpiSize(55),
								forced_height = utils.ui.dpiSize(55),
								widget = wibox.container.background,
								create_callback = function(self, c, index, objects) --luacheck: no unused
									self:get_children_by_id("clienticon")[1].client = c
									-- 添加单击事件处理函数
									self:buttons(gears.table.join(awful.button({}, 1, nil, function()
										c:jump_to()
										c:raise()
